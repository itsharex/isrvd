      ...
      <div class="bg-slate-50 border-b border-slate-200 rounded-t-2xl px-4 md:px-6 py-3">
        ...
        <div class="hidden md:flex items-center justify-between">
          ...
          <div class="flex items-center gap-2">
            ...
            <button v-if="actions.hasPerm('POST /api/docker/image/:id/action')" @click="buildModalRef?.show()" class="px-3 py-1.5 rounded-lg bg-blue-500 hover:bg-blue-600 text-white text-xs font-medium flex items-center gap-1.5 transition-colors">
              <i class="fas fa-hammer"></i>构建
            </button>
            <button v-if="actions.hasPerm('POST /api/docker/images/pull')" @click="pullModalRef?.show()" class="px-3 py-1.5 rounded-lg bg-blue-500 hover:bg-blue-600 text-white text-xs font-medium flex items-center gap-1.5 transition-colors">
        ...
        <div class="block md:hidden">
          <div class="flex items-center justify-between mb-3">
            ...
            <div class="flex items-center gap-1 flex-shrink-0">
              ...
              <button v-if="actions.hasPerm('POST /api/docker/image/:id/action')" @click="buildModalRef?.show()" class="w-9 h-9 rounded-lg bg-blue-500 hover:bg-blue-600 flex items-center justify-center text-white transition-colors" title="构建">
                <i class="fas fa-hammer text-sm"></i>
              </button>
              <button v-if="actions.hasPerm('POST /api/docker/images/pull')" @click="pullModalRef?.show()" class="w-9 h-9 rounded-lg bg-blue-500 hover:bg-blue-600 flex items-center justify-center text-white transition-colors" title="拉取">
            ...
            <tr v-for="img in images" :key="img.id" class="hover:bg-slate-50 transition-colors">
              ...
              <td class="px-4 py-3">
                <div class="flex justify-end items-center gap-0.5">
                  <button v-if="actions.hasPerm('GET /api/docker/image/:id')" @click="$router.push('/docker/image/' + img.id)" class="btn-icon text-slate-600 hover:bg-slate-50" title="查看详情">
                  ...
                  <button @click="pullImage(img)" v-if="actions.hasPerm('POST /api/docker/images/pull')" class="btn-icon text-emerald-600 hover:bg-emerald-50" title="拉取（更新）">
                  ...
                  <button @click="handleImageAction(img, 'remove')" v-if="actions.hasPerm('POST /api/docker/image/:id/action')" class="btn-icon text-red-600 hover:bg-red-50" title="删除">
