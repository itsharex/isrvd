    ...
    <div class="hidden md:flex md:items-center justify-between">
      ...
      <div v-if="container" class="flex items-center gap-2">
        <div class="flex gap-1 bg-slate-100 p-1 rounded-lg">
          ...
          <button v-if="actions.hasPerm('docker', true)" @click="switchTab('docker-container-exec')" :class="['px-3 py-1 text-xs font-medium rounded-md transition-all duration-200 flex items-center gap-1.5', activeTab === 'docker-container-exec' ? 'bg-white text-emerald-600 shadow-sm' : 'text-slate-500 hover:text-slate-700']">
