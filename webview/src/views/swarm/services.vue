      ...
      <div class="bg-slate-50 border-b border-slate-200 rounded-t-2xl px-4 md:px-6 py-3">
        ...
        <div class="hidden md:flex items-center justify-between">
          ...
          <div class="flex items-center gap-2">
            ...
            <button v-if="actions.hasPerm('POST /api/swarm/services')" @click="openCreateModal" class="px-3 py-1.5 rounded-lg bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-medium flex items-center gap-1.5 transition-colors">
        ...
        <div class="flex md:hidden items-center justify-between">
          ...
          <div class="flex items-center gap-1 flex-shrink-0">
            ...
            <button v-if="actions.hasPerm('POST /api/swarm/services')" @click="openCreateModal" class="w-9 h-9 rounded-lg bg-emerald-500 hover:bg-emerald-600 flex items-center justify-center text-white transition-colors" title="创建">
              ...
              <tr v-for="svc in services" :key="svc.id" class="hover:bg-slate-50 transition-colors">
                ...
                <td class="px-4 py-3">
                  <div class="flex justify-end items-center gap-0.5">
                    <button v-if="actions.hasPerm('GET /api/swarm/services/:id')" @click="$router.push({ name: 'swarm-service-info', params: { id: svc.id } })" class="btn-icon text-slate-600 hover:bg-slate-50" title="详情"><i class="fas fa-circle-info text-xs"></i></button>
                    <button v-if="actions.hasPerm('GET /api/swarm/services/:id/logs')" @click="$router.push({ name: 'swarm-service-logs', params: { id: svc.id } })" class="btn-icon text-slate-600 hover:bg-slate-50" title="日志"><i class="fas fa-file-lines text-xs"></i></button>
                    <button v-if="svc.mode === 'replicated' && actions.hasPerm('POST /api/swarm/services/:id/action')" @click="openScaleModal(svc)" class="btn-icon text-indigo-600 hover:bg-indigo-50" title="扩缩容"><i class="fas fa-up-right-and-down-left-from-center text-xs"></i></button>
                    <button v-if="actions.hasPerm('POST /api/swarm/services/:id/force-update')" @click="handleRedeploy(svc)" class="btn-icon text-blue-600 hover:bg-blue-50" title="强制重部署"><i class="fas fa-arrows-rotate text-xs"></i></button>
                    <button v-if="actions.hasPerm('POST /api/compose/swarm/:name/redeploy')" @click="openEditModal(svc)" class="btn-icon text-blue-600 hover:bg-blue-50" title="编辑"><i class="fas fa-pen text-xs"></i></button>
                    <button v-if="actions.hasPerm('POST /api/swarm/services/:id/action')" @click="handleServiceRemove(svc)" class="btn-icon text-red-600 hover:bg-red-50" title="删除"><i class="fas fa-trash text-xs"></i></button>
                  </div>
                </td>
              </tr>
            </tbody>
        ...
        <div class="md:hidden space-y-3 p-4">
          ...
          >
            ...
            <div class="flex flex-wrap gap-1 pt-2 border-t border-slate-100">
              <button v-if="actions.hasPerm('GET /api/swarm/services/:id')" @click="$router.push({ name: 'swarm-service-info', params: { id: svc.id } })" class="btn-icon text-slate-600 hover:bg-slate-50" title="详情">
                <i class="fas fa-circle-info text-xs"></i><span class="text-xs ml-1">详情</span>
              </button>
              <button v-if="actions.hasPerm('GET /api/swarm/services/:id/logs')" @click="$router.push({ name: 'swarm-service-logs', params: { id: svc.id } })" class="btn-icon text-slate-600 hover:bg-slate-50" title="日志">
                <i class="fas fa-file-lines text-xs"></i><span class="text-xs ml-1">日志</span>
              </button>
              <button v-if="svc.mode === 'replicated' && actions.hasPerm('POST /api/swarm/services/:id/action')" @click="openScaleModal(svc)" class="btn-icon text-indigo-600 hover:bg-indigo-50" title="扩缩容">
                <i class="fas fa-up-right-and-down-left-from-center text-xs"></i><span class="text-xs ml-1">扩缩容</span>
              </button>
              <button v-if="actions.hasPerm('POST /api/swarm/services/:id/force-update')" @click="handleRedeploy(svc)" class="btn-icon text-blue-600 hover:bg-blue-50" title="强制重部署">
                <i class="fas fa-arrows-rotate text-xs"></i><span class="text-xs ml-1">重部署</span>
              </button>
              <button v-if="actions.hasPerm('POST /api/compose/swarm/:name/redeploy')" @click="openEditModal(svc)" class="btn-icon text-blue-600 hover:bg-blue-50" title="编辑">
                <i class="fas fa-pen text-xs"></i><span class="text-xs ml-1">编辑</span>
              </button>
              <button v-if="actions.hasPerm('POST /api/swarm/services/:id/action')" @click="handleServiceRemove(svc)" class="btn-icon text-red-600 hover:bg-red-50" title="删除">
